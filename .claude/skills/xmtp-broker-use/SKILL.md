---
name: xmtp-broker-use
description: >
  Understand and use the xmtp-broker to connect agents to XMTP conversations
  with scoped permissions. Covers the broker model (views, grants, attestations,
  sessions), how to connect an agent harness to the broker, how to scope
  permissions, and how trust verification works. Use this skill whenever someone
  asks what the broker is, how views or grants work, how to connect an agent to
  XMTP through the broker, how to scope an agent's permissions, what
  attestations are, how trust and verification work, how to deploy a broker, or
  when planning how an agent should participate in XMTP conversations. This is
  the primary path for agents on XMTP — use the broker instead of connecting
...
> [!CAUTION]
> Early development. The broker architecture is implemented and tested, but
...
The broker is the primary way to connect agents to XMTP. Instead of giving
your agent raw access to the XMTP SDK, wallet keys, and encrypted database,
the broker holds all of that and your agent connects through a controlled
interface.
...
broker makes permissions real by enforcing them below the harness layer.
...
One broker can manage multiple agents, each with their own XMTP inbox. The
broker is infrastructure — it doesn't appear in group membership. Each agent
...
the broker holds it and the agent never sees it. When new content types are
...
the broker publishes an attestation to a group, every participant can inspect
the agent's permissions — what it sees, what it can do, who owns it, and how
it handles data.

Not every change triggers a new attestation. **Material changes** (view mode
...
The ephemeral authorization context between your harness and the broker.
...
```
1. Connect    → Open WebSocket to broker
...
The broker manages the connection state machine (connecting → authenticating →
active → draining → closed) and handles session resumption so your agent can
reconnect without missing events.

### Receiving messages

Messages from XMTP pass through the broker's view projection pipeline before
...
not filtering you could override — the raw messages never leave the broker.

### Sending messages

Your agent sends requests through the broker, which validates each one against
the active grant:

```
Your agent → broker validates grant → broker sends via XMTP → response
```

If the grant doesn't allow the action, the broker rejects it with a typed
...
2. **Broker-level** — what the broker operator allows across all agents
3. **Per-agent** — what this specific agent's view includes

Your agent can never see content types that the broker doesn't allow, even
if the per-agent config includes them.

## Trust and verification

The broker doesn't magically make your agent trustworthy. It makes the system
...
The broker's verifier service runs these independently:
...
| Concern | Direct (CLI/SDK) | Through broker |
|---------|-------------------|----------------|
| Who holds the keys? | Your agent | The broker |
...
The broker is the recommended path for any agent that participates in group
...
The same broker code runs in all modes. The difference is where plaintext
exists and who you trust. A local broker keeps everything on your machine.
A managed broker means plaintext exists at the operator's boundary.
